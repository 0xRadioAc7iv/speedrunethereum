...
contract DEX {
	...
	function init(uint256 tokens) public payable returns (uint256) {
		...
		require(tokens > 0, "Need to send more than 0 tokens");
	...
	function ethToToken() public payable returns (uint256 tokenOutput) {
		...
		uint256 tokensBought = price(msg.value, ethBalance, tokenBalance);

		bool sent = token.transfer(msg.sender, tokensBought);
		require(sent, "Error transferring $BAL");
	...
	function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
		require(tokenInput > 0, "Must send more than 0 tokens");

		uint256 userTokenBalance = token.balanceOf(msg.sender);
		require(userTokenBalance >= tokenInput, "Insufficient Token Balance");

		uint256 ethBalance = address(this).balance;
		uint256 tokenBalance = token.balanceOf(address(this));
		uint256 ethBought = price(userTokenBalance, tokenBalance, ethBalance);

		bool sentToken = token.transferFrom(
			msg.sender,
			address(this),
			tokenInput
		);
		require(sentToken, "Error transferring $BAL");

		(bool sent, ) = msg.sender.call{ value: ethBought }("");
		require(sent, "Error transferring ETH");
	...
	function deposit() public payable returns (uint256 tokensDeposited) {
		...
		tokenDeposit = ((msg.value * tokenBalance) / ethBalance) + 1;
	...
	function withdraw(uint256 amount) public returns (uint256, uint256) {
		...
		uint256 ethWithdrawn = (amount * ethBalance) / totalLiquidity;
		uint256 tokenAmount = (amount * tokenBalance) / totalLiquidity;
		...
		require(tokenSent, "Error transferring $BAL");
